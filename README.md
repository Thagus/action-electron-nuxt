...
2. If you need to compile code (e.g. TypeScript to JavaScript or Sass to CSS), make sure this is done using a **`build` script in your `package.json` file**.
     ...
     release:
       ...
       steps:
         ...
         - name: Build/release Electron app
           uses: thagus/action-electron-nuxt@v1
           with:
...
Add the following options to your workflow's existing `action-electron-nuxt` step:
...
If you are building/releasing your Linux app for Snapcraft (which is `electron-builder`'s default), you will additionally need to install and sign in to Snapcraft. This can be done using an `action-snapcraft` step before the `action-electron-nuxt` step:
...
2.  In your workflow file, add the following step before your `action-electron-nuxt` step:
...
3.  Pass the following environment variables to `action-electron-nuxt`:
